...
- 所有生产 DDL 必须遵循 [Migration Version Policy](docs/00-product-baseline/database/Migration-Version-Policy.md)，进入对应 Owner 实现模块的版本化 `<owner-module>/src/main/resources/db/migration/V<monotonic-version>__<description>.sql`，由 `application` 聚合运行时 classpath 并交给 `r2dbc-migrate` 执行。版本号基于 UTC+8 时间生成，并受全局单调序列约束；各模块共享该序列；已进入共享/正式环境的 Migration 不得原地修改，修复只能追加新 Migration。
