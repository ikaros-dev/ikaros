...
- 注册、登录成功/失败/拒绝和用户主动失效令牌均写入结构化审计；未认证登录尝试使用无主体 ID 的 `ANONYMOUS` 语义并受数据库约束保护
- 管理员用户、角色、权限、用户角色和令牌安全操作统一记录 `ADMIN` 高风险审计；新增自定义角色编辑/删除、内置角色保护、绑定冲突保护及无权限 `DENIED` 审计
...
- 同步 App Runtime Foundation 文档实施状态：区分已实现 Registry/lifecycle/client/permission slice、schema-only Dependency/Migration，以及仍待实现的 Package、Discovery、AppAuthorizationGrant、PKCE 和 `DELETE_APP_DATA`
- 注册和登录
- 完成系统访问控制后台：用户管理、角色与权限管理、用户角色关联管理
- 用户支持新增、详情、编辑、分页查询和软删除；软删除使用 `is_del`（`0` 未删除、`1` 已删除）并在后端统一过滤
- 完善 Step-up 验证：Email OTP / SMS OTP 签发 `SVL-2`，后台管理统一使用 Email OTP，并支持 Email OTP 复用窗口
- 增强高风险权限策略：SMS 未启用时 `storage.provider.manage` 降级为 `SVL-2`，启用时保持 `SVL-3`
- 统一角色编码、权限描述、权限保存和前端错误详情展示
- 控制台菜单按登录用户权限动态过滤，统一以后端 `system.*` 权限 Key 为准；管理权限可进入对应的只读菜单
- 修复无可见菜单时侧边栏持续加载，以及登录后菜单为空导致标签页初始化异常的问题
