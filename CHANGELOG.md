...
- 新增内容类型与检索契约，为视频、音乐、图片内容提供统一的数据模型与查询能力
- 服务启动完成后输出中英文双语摘要，集中展示系统版本、访问地址、已加载插件、运行状态

### 修复与优化

- 优化本地文件源目录选择、扫描、显式导入流程，并收紧导入条目类型校验
- 优化批量关联歌单歌曲的性能
- 限制插件附件驱动仅使用 `CUSTOM` 类型，避免插件配置与内置驱动类型冲突

### 安全修复

- 禁用 TOTP 二步验证时校验账户密码，并改用请求体传递参数，避免敏感信息出现在请求地址中

## 控制台

### 新功能

- 新增统一内容浏览界面，提供视频、音乐、图片内容页面
- 重构文件源管理入口，统一目录选择、扫描预览、显式导入、操作反馈流程
- 调整仪表盘入口与后台布局，新增共享内容浏览外壳和内容引导，修正统计数据口径
- 优化 TOTP 二步验证开启入口和设置界面，完善操作反馈

### 修复与优化

- 优化插件状态展示与启停操作布局
- 统一用户表单、仪表盘错误状态、刷新按钮交互，使用表格原生排序能力
- 修复文件源类型切换后表单状态异常
- 将认证状态限制在当前会话，避免跨会话保留敏感认证状态

## 构建与工程质量

- 全面采用 JSpecify 空值注解，并为 API、服务端、持久化包启用默认非空约束
- 新增生产包默认非空覆盖和 Element Plus 原生控件使用门禁，清理构建警告与弃用调用
- 补充 CI 提交类型规范

## 测试

- 补充内容浏览、文件源管理、目录选择、仪表盘、通用表单和启动摘要的测试
- 补充 TOTP 禁用密码校验、插件附件驱动类型校验、空值注解约束及相关服务端回归测试
...
# 1.2.3
  ...
  - 附件上传、分片合并、本地驱动扫描、媒体绑定统一校验真实文件格式，阻止伪造扩展名或媒体内容不匹配的文件进入系统
  - 附件流响应根据检测到的真实媒体格式设置内容类型，避免浏览器将不可信内容按错误类型解析
  - 收紧视频字幕自动关联和本地目录扫描规则，仅处理符合白名单且真实格式有效的媒体文件

  - 新增媒体文件格式白名单和统一策略，覆盖视频、音频、字幕、图片及 SVG 等文件类别
  - 新增有界媒体文件检测器，可通过文件头和有限内容识别常见媒体格式，并保留扩展名提示与真实格式的判定结果
  - 普通上传与分片上传接入流式媒体校验，避免为验证文件格式而完整读取大文件
  - 本地附件驱动刷新、目录绑定扫描、附件内容检查接入真实格式检测

  ## 控制台

  ### 优化

  - 附件上传、选择、多选界面统一使用媒体文件白名单，减少无效文件选择
  - 附件与剧集媒体预览根据真实媒体类别处理展示，优化视频播放器和附件详情的媒体类型判断
  - 新增前端媒体格式工具，统一文件扩展名、媒体类别、可接受文件类型的判断逻辑

  ## API 与兼容性

  - 统一附件上传和媒体流接口的内容类型处理，前端客户端适配流式响应的二进制数据类型

  ## 测试

  - 补充媒体格式策略、文件头检测、上传流式校验、附件流响应、本地驱动扫描、媒体绑定的测试
