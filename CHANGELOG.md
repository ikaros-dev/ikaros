...
- 新增内容类型与检索契约，为视频、音乐和图片内容提供统一的数据模型与查询能力
- 服务启动完成后输出中英文双语摘要，集中展示系统版本、访问地址、已加载插件和运行状态

### 修复与优化

- 优化本地文件源目录选择、扫描和显式导入流程，并收紧导入条目类型校验
- 优化歌单歌曲关联的批量写入性能
...
- 新增统一内容浏览界面，提供视频、音乐和图片内容页面
- 重构文件源管理入口，统一目录选择、扫描预览、显式导入和操作反馈流程
- 仪表盘新增内容引导，并修正统计数据口径

### 修复与优化

- 优化 TOTP 二步验证开启入口，改用可交互的圆形图标并完善操作反馈
- 优化插件状态展示与启停操作布局
- 统一用户表单、仪表盘错误状态和刷新按钮交互，使用表格原生排序能力
- 修复文件源类型切换后表单状态异常，并还原登录页输入框样式

## 构建与工程质量

- 全面采用 JSpecify 空值注解，并为 API、服务端和持久化包启用默认非空约束
