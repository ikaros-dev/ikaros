...
public class StepUpVerificationController {
    ...
    @ApiResponses({
        ...
        @ApiResponse(responseCode = "404", description = "活跃用户不存在", content = @Content),
    ...
    public Mono<ResponseEntity<VerificationChallengeView>> issue(
        ...
        return stepUpService.issueEmailOtp(userId).map(view -> ResponseEntity.accepted().body(view));
    }

    /**
     * 验证当前用户绑定的 OTP 并返回短期增强 JWT。
     *
     * @param userId 当前认证用户标识
     ...
     * @return 标准验证结果
     */
    @Operation(summary = "完成增强验证", description = "只接受当前用户绑定且用途为 LOGIN_STEP_UP 的 OTP 挑战。"
        + "成功后返回短期 Purpose-bound Verification Grant；权限仍由 RBAC 单独决定。")
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "增强验证成功并返回 Verification Grant"),
        @ApiResponse(responseCode = "404", description = "用户或验证挑战不存在", content = @Content),
