...
public class PersistentDriveService implements DriveService {
    ...
    @Override public Mono<DriveNodeView> createNode(UUID actor, UUID sid, CreateDriveNodeRequest req) {
        return ownedSpace(actor,sid).flatMap(s -> { UUID parent = req.parentId()==null?s.rootNodeId():req.parentId(); return nodes.findByIdAndDriveSpaceId(parent,sid).switchIfEmpty(Mono.error(new NotFoundException("父节点不存在"))).flatMap(p -> {
