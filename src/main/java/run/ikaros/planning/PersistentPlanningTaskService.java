...
public class PersistentPlanningTaskService implements PlanningTaskService {
    ...
    @Override public Mono<PlanningTaskView> create(UUID owner,CreatePlanningTaskRequest req){Instant now=Instant.now();return repository.save(new PlanningTaskEntity(null,owner,req.title().trim(),req.description(),PlanningTaskStatus.INBOX,req.priority()==null?PlanningTaskPriority.NONE:req.priority(),req.deadline(),req.projectId(),req.parentTaskId(),null,now,now,null)).map(this::view);}
    ...
    private PlanningTaskView view(PlanningTaskEntity t){return new PlanningTaskView(t.id(),t.ownerId(),t.title(),t.description(),t.status(),t.priority(),t.deadline(),t.projectId(),t.parentTaskId(),t.completedAt(),t.createdAt(),t.updatedAt(),t.version()==null?0:t.version());}
}
