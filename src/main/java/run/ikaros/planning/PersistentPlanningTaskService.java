...
public class PersistentPlanningTaskService implements PlanningTaskService {
    ...
    public PersistentPlanningTaskService(PlanningTaskRepository repository, PlanningProjectSectionRepository sections){this.repository=repository;this.sections=sections;}
    @Override public Mono<PlanningTaskView> create(UUID owner,CreatePlanningTaskRequest req){validateSchedule(req.scheduledStart(),req.scheduledEnd());validateDuration(req.estimatedDurationMinutes());return validSection(req.projectId(),req.sectionId()).then(Mono.defer(()->{Instant now=Instant.now();return repository.save(new PlanningTaskEntity(null,owner,req.title().trim(),req.description(),PlanningTaskStatus.INBOX,req.priority()==null?PlanningTaskPriority.NONE:req.priority(),req.important(),req.urgent(),req.scheduledStart(),req.scheduledEnd(),req.deadline(),req.estimatedDurationMinutes(),req.projectId(),req.sectionId(),req.parentTaskId(),null,now,now,null)).map(this::view);}));}
    ...
    private Mono<Void> validSection(UUID projectId, UUID sectionId){if(sectionId==null)return Mono.empty();return sections.findById(sectionId).filter(s->projectId!=null&&s.projectId().equals(projectId)).switchIfEmpty(Mono.error(new ConflictException("Section 不属于该 Project"))).then();}
