...
@RestController
@RequestMapping("/api/planning/milestones")
public class PlanningMilestoneController {
    ...
    @PostMapping("/{milestoneId}/status/{status}") public Mono<ResponseEntity<PlanningMilestoneView>> status(@RequestHeader("X-Ikaros-Actor-Id") UUID ownerId, @PathVariable UUID milestoneId, @PathVariable PlanningMilestoneStatus status, @RequestHeader(value="If-Match",required=false) String ifMatch) { return service.changeStatus(ownerId, milestoneId, status, IfMatchVersion.parse(ifMatch)).map(view->ResponseEntity.ok().eTag(IfMatchVersion.etag(view.version())).body(view)); }
}
