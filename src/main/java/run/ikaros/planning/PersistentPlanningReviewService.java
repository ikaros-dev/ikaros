...
public class PersistentPlanningReviewService implements PlanningReviewService {
    ...
    @Override public Flux<PlanningReviewView> list(UUID ownerId, PlanningReviewPeriod period) { Flux<PlanningReviewEntity> source = reviews.findAllByOwnerIdOrderByPeriodStartDesc(ownerId); return (period == null ? source : source.filter(r -> r.period() == period)).map(this::view); }
    @Override public Mono<PlanningReviewView> update(UUID ownerId, UUID reviewId, UpdatePlanningReviewRequest request) { validate(request.periodStart(), request.periodEnd()); return owned(ownerId, reviewId).flatMap(old -> { if ((old.version() == null ? 0 : old.version()) != request.expectedVersion()) return Mono.error(new ConflictException("Review 版本冲突")); return reviews.save(new PlanningReviewEntity(old.id(), old.ownerId(), old.period(), request.periodStart(), request.periodEnd(), request.note().trim(), request.wins(), request.challenges(), request.nextFocus(), old.createdAt(), Instant.now(), old.version())); }).map(this::view); }
