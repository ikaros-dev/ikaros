...
public class PersistentOfflineCacheService implements OfflineCacheService {
    ...
    @Override public Flux<CacheEntryView> list(UUID user,UUID device){return entries.findAllByUserIdAndDeviceIdOrderByLastAccessedAtDesc(user,device).filter(e->e.state()==CacheEntryState.ACTIVE).map(this::view);}
