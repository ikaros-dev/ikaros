...
@Service public class PersistentMusicPlaylistService implements MusicPlaylistService {
    ...
    @Override public Flux<MusicPlaylistView> list(UUID ownerId) { return playlists.findAllByOwnerIdOrderByUpdatedAtDesc(ownerId).map(this::view); }
    @Override public Flux<MusicPlaylistEntryView> entries(UUID ownerId, UUID playlistId) { return ownedPlaylist(ownerId,playlistId).flatMapMany(p->entries.findAllByPlaylistIdOrderByPositionAsc(playlistId).map(this::entryView)); }
