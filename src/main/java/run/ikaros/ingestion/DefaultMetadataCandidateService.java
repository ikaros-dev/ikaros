...
@Service public class DefaultMetadataCandidateService implements MetadataCandidateService {
 ...
 public Mono<List<MetadataCandidateView>> list(UUID owner,UUID resource){return owned(owner,resource).thenMany(candidates.findAllByResourceIdOrderByCreatedAtDesc(resource)).map(this::view).collectList();}
