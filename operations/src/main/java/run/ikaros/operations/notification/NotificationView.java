...
public record NotificationView(
    ...
    String priority, String status, UUID taskId, UUID resourceId, Instant createdAt, Instant readAt,
    Instant archivedAt
) {
    static NotificationView from(NotificationEntity value) {
        return new NotificationView(value.id(), value.eventId(), value.recipientId(), value.source(),
            value.eventType(), value.title(), value.body(), value.priority(), value.status(), value.taskId(),
            value.resourceId(), value.createdAt(), value.readAt(), value.archivedAt());
    }
}
