...
async function load() { if (!resourceId.value) { notify('资源 ID 不存在'); return }; if (!actorId) { notify('未配置当前用户身份，无法加载资源'); return }; loading.value = true; try { const result = await api.getResource(resourceId.value, actorId); title.value = result.title || result.id; type.value = result.resource_type || '其他'; lifecycle.value = result.lifecycle || '未知' } catch (error) { if (!(error instanceof ApiError && import.meta.env.DEV && error.status === 404)) notify('资源详情加载失败，请检查登录状态或后端服务') } finally { loading.value = false } }
