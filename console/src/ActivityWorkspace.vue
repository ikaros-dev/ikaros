...
async function load() { loading.value = true; error.value = ''; if (!actorId) { records.value = []; error.value = '未配置当前用户身份，无法加载活动'; loading.value = false; return } try { records.value = await api.listRecentActivity(actorId, 50) } catch { error.value = '活动 API 加载失败，请检查后端服务'; records.value = [] } finally { loading.value = false } }
onMounted(load)
</script>
