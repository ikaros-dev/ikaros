...
const expanded = ref<string[]>(storedPreferences.expanded || []); const theme = ref<Theme>(storedPreferences.theme || 'system'); const drawer = ref(false); const dialog = ref(''); const toast = ref(''); const query = ref(String(route.query.q || '')); const statusFilter = ref('全部状态'); const selectedTab = ref('概览'); const loading = ref(false); const apiConnected = ref(false); const errorState = ref<{ status: number; title: string; detail: string } | null>(null)
