...
const fetchAuthorities = async () => {
  const {data} = await apiClient.authority.getAuthoritiesByCondition({
    ...
    type: authorityCondition.value.type as "ALL" | "API" | "APIS" | "MENU" | "URL" | "OTHERS",
...
<template>
  <div>
    <el-dialog v-model="authorityDialogVisible" title="Role Authorities Multi Select Dialog" width="70%"
               @open="fetchAuthorities">
      <el-form :inline="true" :model="authorityCondition">
        <el-form-item label="Type">
          <el-select v-model="authorityCondition.type" placeholder="Select Authiroty Type"
                     @change="fetchAttachmentsWithPageOne">
            <el-option v-for="auth in authorityTypes" :key="auth" :label="auth" :value="auth"/>
          </el-select>
        </el-form-item>
        <el-form-item label="Target">
          <el-input v-model="authorityCondition.target" clearable @change="fetchAttachmentsWithPageOne"/>
        </el-form-item>
        <el-form-item label="Authority">
          <el-input v-model="authorityCondition.authority" clearable @change="fetchAttachmentsWithPageOne"/>
        </el-form-item>
      </el-form>

      <el-pagination v-model:page-size="authorityCondition.size" v-model:current-page="authorityCondition.page"
                     background :total="authorityCondition.total" :pager-count="5"
                     layout="total, sizes, prev, pager, next, jumper" @current-change="onCurrentPageChange"
                     @size-change="onSizeChange"/>

      <el-table :data="autiorities" style="width: 100%" @selection-change="onAuthTabSelectChange">
      ...
      <el-table-column fixed="right" label="Operations" min-width="120">
        <template #default="scope">
          <el-popconfirm width="300" confirm-button-text="Delete" cancel-button-text="Cancel"
                         ...
                         title="Are you sure to delete this authority?" @confirm="deleteRoleAuthority(scope.row.id)">
            <template #reference>
              <el-button type="danger" :disabled="scope.row.id === 1">Delete
              </el-button>
            </template>
          </el-popconfirm>
        </template>
