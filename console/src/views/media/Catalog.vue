...
async function loadQueueEntries(id = queueId.value) {
  ...
  try { const result = await http.get<unknown, unknown>(`/music/queues/${id.trim()}/entries`) as Row[]; queueEntries.value = Array.isArray(result) ? result : []; return queueEntries.value; }
