...
<template>
  <main class="p-4 md:p-6">
    ...
    <el-card shadow="never" class="mt-4">
      <template v-if="tab === 'policy'"><div class="grid grid-cols-1 md:grid-cols-2 gap-4"><el-card v-for="item in [{name:'Password / Login Policy',value:'由服务端生效',hint:'登录密码校验与失败策略'},{name:'MFA 要求',value:'由安全策略提供',hint:'高风险操作可能要求二次验证'},{name:'JWT Lifetime',value:'由认证服务提供',hint:'Access Token 过期后使用 Refresh Token'},{name:'Login Rate Limit',value:'由认证服务提供',hint:'失败次数和锁定策略'}]" :key="item.name" shadow="never"><div class="font-medium">{{ item.name }}</div><div class="text-lg mt-3">{{ item.value }}</div><div class="text-sm text-[var(--el-text-color-secondary)] mt-2">{{ item.hint }}</div></el-card></div><el-divider /><el-form label-position="top"><el-form-item label="验证用途"><el-select v-model="purpose"><el-option label="登录增强验证" value="LOGIN_STEP_UP" /><el-option label="修改安全设置" value="CHANGE_SECURITY_SETTING" /><el-option label="导出安全资料" value="EXPORT_SECURE_VAULT" /></el-select></el-form-item><div v-if="challenge" class="flex flex-wrap items-end gap-3"><el-form-item label="Challenge ID" class="flex-1 min-w-64"><el-input :model-value="challenge.id" readonly /></el-form-item><el-form-item label="状态"><el-tag>{{ challenge.status || 'ISSUED' }}</el-tag></el-form-item><el-form-item label="有效期至"><el-input :model-value="challenge.expiresAt || '—'" readonly /></el-form-item><el-form-item label="验证码"><el-input v-model="code" maxlength="6" placeholder="输入邮件中的 6 位验证码" /></el-form-item><el-button type="primary" :disabled="challenge.status !== 'ISSUED'" :loading="verifying" @click="verify">验证</el-button><el-button v-if="challenge.status === 'ISSUED'" @click="cancel">取消挑战</el-button></div><el-alert v-else title="点击“发起验证”后，验证码会通过已配置的认证渠道发送；页面不会显示验证码。" type="info" show-icon :closable="false" /></el-form></template>
