...
const archived = ref<Row[]>([]);

async function load() {
  loading.value = true; error.value = "";
  try {
    const [restore, gc, budgetResult, archivedResult] = await Promise.all([http.get("/restore-requests"), http.get("/storage/gc/candidates"), http.get("/admin/restore-budget-policy"), http.get("/resources", { params: { lifecycle_status: "ARCHIVED", page: 0, size: 100 } })]);
    ...
    const archivedPage: any = archivedResult;
    archived.value = Array.isArray(archivedPage) ? archivedPage : (archivedPage?.items || archivedPage?.content || []);
...
<template>
  <main class="p-4 md:p-6">
    ...
    <el-card shadow="never"><el-tabs v-model="tab">
      <el-tab-pane label="已归档资源" name="archived"><el-empty v-if="!archived.length" description="暂无已归档资源" /><el-table v-else :data="archived" stripe><el-table-column prop="id" label="Resource ID" min-width="240" /><el-table-column prop="primaryTitle" label="标题" min-width="240" /><el-table-column prop="type" label="类型" width="130" /><el-table-column prop="lifecycle" label="状态" width="130" /><el-table-column prop="updatedAt" label="归档时间" min-width="180" /></el-table></el-tab-pane>
