...
async function loadMembers() { if (!editTargetId.value) { members.value = []; return; } memberLoading.value = true; try { const result = await http.get<unknown, unknown>(`/collections/${editTargetId.value}/resources`); members.value = Array.isArray(result) ? result as { resourceId: string; position: number }[] : []; } catch (e: any) { error.value = e?.response?.data?.detail || e?.message || "集合成员加载失败"; } finally { memberLoading.value = false; } }
