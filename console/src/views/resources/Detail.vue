...
async function resolveMetadataCandidate(row: Record<string, any>, resolution: "APPLY" | "REJECT") {
  if (!row.id || String(row.status).toUpperCase() !== "PENDING") return;
  if (resolution === "REJECT" && !window.confirm("确认拒绝这个外部元数据候选吗？")) return;
  try { await http.post(`/ingestion/resources/metadata-candidates/${row.id}/resolution`, { data: { resolution } }); await load(); }
  catch (e: any) { error.value = e?.response?.data?.detail || e?.message || "处理元数据候选失败"; }
}
