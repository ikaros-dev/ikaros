...
async function load() {
  ...
  try {
    const [result, files, metadataResult] = await Promise.all([http.get(`/resources/${route.params.resourceId}`), http.get(`/resources/${route.params.resourceId}/attachments`), http.get(`/resources/${route.params.resourceId}/metadata`)]);
    resource.value = result; attachments.value = Array.isArray(files) ? files : []; metadata.value = Array.isArray(metadataResult) ? metadataResult : [];
