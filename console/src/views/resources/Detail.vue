...
async function load() {
  ...
  try {
    ...
    const [stateResult, tagResult, activityResult] = await Promise.all([http.get(`/resources/${route.params.resourceId}/user-state`).catch(() => null), http.get(`/resources/${route.params.resourceId}/tags`), http.get(`/activity?limit=200`)]);
    resource.value = result; attachments.value = Array.isArray(files) ? files : []; metadata.value = Array.isArray(metadataResult) ? metadataResult : []; relations.value = Array.isArray(relationResult) ? relationResult : []; userState.value = stateResult || { favorite: false, rating: null, statusCode: "", progressValue: null, progressUnit: "" }; tags.value = Array.isArray(tagResult) ? tagResult : []; activities.value = (Array.isArray(activityResult) ? activityResult : []).filter((item: any) => String(item.resourceId) === String(route.params.resourceId));
