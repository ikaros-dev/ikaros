...
const titleForm = ref({ locale: "en-US", title: "", primary: false });
...
function beginTitle() { titleForm.value = { locale: "en-US", title: "", primary: false }; titleDialog.value = true; }
async function saveTitle() {
  if (!resource.value || !titleForm.value.title.trim()) { error.value = "标题不能为空"; return; }
  try { await http.request("put", `/resources/${resource.value.id}/titles`, { data: { locale: titleForm.value.locale, title: titleForm.value.title.trim(), primary: titleForm.value.primary, kind: "TITLE" } }); titleDialog.value = false; await load(); }
