...
<template>
  <main class="p-4 md:p-6">
    <div class="flex justify-between items-start mb-6"><div><p class="text-sm text-[var(--el-text-color-secondary)]">资源中心 / 资源详情</p><h1 class="mt-2 text-2xl font-semibold">{{ resource?.primaryTitle || resource?.title || route.params.resourceId }}</h1><p class="mt-1 text-[var(--el-text-color-secondary)]">查看资源元数据、附件关系和生命周期状态。</p></div><div class="flex gap-2"><el-button @click="$router.back()">返回</el-button><el-button :loading="loading" @click="load">刷新</el-button><el-button v-if="resource && resource.lifecycle === 'ACTIVE'" type="warning" @click="archive">归档</el-button><el-button v-if="resource" type="primary" @click="beginEdit">编辑元数据</el-button><el-button v-if="resource" @click="$router.push(`/storage-center/attachments?resourceId=${route.params.resourceId}`)">查看附件</el-button></div></div>
