...
async function load() {
  ...
  error.value = "";
  try {
    const result = await http.get<unknown, unknown>("/communications/notifications", {
      params: {
        status: readState.value === "all" ? undefined : readState.value,
    ...
    rows.value = (Array.isArray(result) ? result : page?.items || page?.content || []) as Notification[];
  } catch (e: any) {
    error.value = e?.response?.data?.detail || e?.message || "通知加载失败";
  } finally {
    loading.value = false;
  }
}
...
async function markRead(row: Notification) {
  if (!row.id || row.status === "READ" || actionLoading.value) return;
  actionLoading.value = row.id;
  error.value = "";
  try {
    await http.post(`/communications/notifications/${row.id}/actions/read`);
    row.status = "READ";
  } catch (e: any) {
...
<template>
  <main class="p-4 md:p-6">
    <div class="flex justify-between items-start mb-6">
      <div>
        <h1 class="text-2xl font-semibold">通知中心与投递</h1>
        <p class="mt-1 text-[var(--el-text-color-secondary)]">查看授权范围内通知，并管理安全投递策略。</p>
      </div>
      <el-button :loading="loading" @click="load">刷新</el-button>
    </div>
    <el-alert v-if="error" :title="error" type="warning" show-icon :closable="false" class="mb-4" />
    <el-tabs v-model="tab">
      <el-tab-pane label="通知中心" name="center" />
      <el-tab-pane label="投递规则" name="rules" />
    ...
    <el-card shadow="never" class="mt-4">
      <template v-if="tab === 'center'">
        <div class="flex flex-wrap gap-3 mb-4">
          ...
          <el-select v-model="source" placeholder="来源" class="w-32" @change="load">
            <el-option label="全部来源" value="all" />
            <el-option label="系统" value="system" />
            <el-option label="任务" value="task" />
          </el-select>
          <el-select v-model="priority" placeholder="优先级" class="w-32" @change="load">
            <el-option label="全部优先级" value="all" />
            <el-option label="普通" value="normal" />
            <el-option label="高" value="high" />
          </el-select>
          <el-button disabled>批量标记已读</el-button>
          <el-button disabled>批量归档</el-button>
        </div>
        <el-empty v-if="!rows.length" description="暂无通知" />
        <el-table v-else :data="rows" stripe>
          <el-table-column prop="source" label="来源" width="150" />
          <el-table-column prop="title" label="标题" min-width="240" />
          <el-table-column prop="preview" label="正文预览" min-width="260" show-overflow-tooltip />
      ...
      <template v-else-if="tab === 'rules'">
        <el-empty description="投递规则尚未接入" />
      </template>
      <template v-else>
        <el-empty description="投递日志尚未接入" />
      </template>
    </el-card>
  </main>
</template>
