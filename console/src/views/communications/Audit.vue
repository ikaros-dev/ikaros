...
const filtered = computed(() => rows.value.filter(item => !query.value || [item.action, item.actorId, item.targetId, item.targetType, item.requestId].some(value => String(value || "").toLowerCase().includes(query.value.toLowerCase()))));
...
async function load() {
  ...
  try {
    const result: any = await http.get("/audit-events", { params: { ...bounds(), actor_id: actorId.value || undefined, request_id: requestId.value.trim() || undefined, page: 0, size: 100 } });
...
<template>
  <main class="p-4 md:p-6">
    ...
    <div class="flex flex-wrap gap-3 mb-4"><el-input v-model="query" clearable placeholder="搜索操作、Actor 或资源 ID" class="max-w-sm" /><el-input v-model="actorId" clearable placeholder="按 Actor UUID 筛选" class="max-w-sm" @keyup.enter="load" /><el-input v-model="requestId" clearable placeholder="按 Request ID 筛选" class="max-w-sm" @keyup.enter="load" /><el-select v-model="range" class="w-32" @change="load"><el-option label="近 7 天" value="7d" /><el-option label="近 30 天" value="30d" /></el-select><el-button @click="load">筛选</el-button></div>
