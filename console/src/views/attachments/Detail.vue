...
const previewVisible = ref(false); const previewLoading = ref(false); const previewUrl = ref(""); const previewError = ref("");
...
async function preview() { previewVisible.value = true; previewLoading.value = true; previewError.value = ""; previewUrl.value = ""; if (!canPreview.value) { previewLoading.value = false; previewError.value = "当前文件类型不支持在线预览"; return; } try { const contract = await http.get<any, any>(`/attachments/${route.params.attachmentId}/preview-url`); previewUrl.value = contract.url; } catch (e: any) { previewError.value = e?.response?.data?.detail || e?.message || "预览授权失败"; } finally { previewLoading.value = false; } }
...
function closePreview() { previewUrl.value = ""; previewError.value = ""; previewVisible.value = false; }
...
<template>
  <main class="p-4 md:p-6">
    ...
    <el-dialog v-model="previewVisible" :title="`预览：${data?.fileName || '附件'}`" width="80%" top="5vh" @closed="closePreview"><el-skeleton v-if="previewLoading" :rows="8" animated/><el-alert v-else-if="previewError" :title="previewError" type="warning" show-icon :closable="false"/><div v-else-if="previewUrl && isImage" class="flex justify-center max-h-[75vh] overflow-auto"><img :src="previewUrl" :alt="data?.fileName" class="max-w-full object-contain" /></div><video v-else-if="previewUrl && isVideo" :src="previewUrl" controls preload="metadata" class="w-full max-h-[75vh]"/><audio v-else-if="previewUrl && isAudio" :src="previewUrl" controls preload="metadata" class="w-full mt-4"/><iframe v-else-if="previewUrl && isDocument" :src="previewUrl" class="w-full h-[75vh] border-0"/></el-dialog>
  </main>
</template>
