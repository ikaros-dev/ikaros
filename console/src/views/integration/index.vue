...
<template>
  <main class="p-4 md:p-6">
    <div class="flex justify-between items-start mb-6"><div><h1 class="text-2xl font-semibold">{{ titles[page] || "集成与自动化" }}</h1><p class="mt-1 text-[var(--el-text-color-secondary)]">外部连接、规则和任务均受权限与审计策略约束。</p></div><div class="flex gap-2"><el-button v-if="page === 'plugins'" type="primary" @click="openInstall">安装插件</el-button><el-button :loading="loading" @click="loadPlugins">刷新</el-button></div></div>
