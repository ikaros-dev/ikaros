...
const readerChapterId = ref(""); const readerWorkId = ref(""); const readerEditionId = ref(""); const readerDirection = ref("LTR"); const readerLayout = ref("SINGLE"); const readerLoading = ref(false); const readerPreferenceSaving = ref(false); const readerProgressSaving = ref(false); const readerError = ref(""); const readerPageIndex = ref(0); const readerSession = ref<Row | null>(null); const readerPages = ref<Row[]>([]); const readerImages = ref<Record<string, string>>({}); const readerImageErrors = ref<Record<string, string>>({});
...
async function openReader() {
  ...
  readerLoading.value = true; error.value = ""; readerError.value = ""; readerImageErrors.value = {}; Object.values(readerImages.value).forEach(url => URL.revokeObjectURL(url)); readerImages.value = {};
  try {
    ...
    tab.value = "reader";
  } catch (e: any) { readerError.value = e?.response?.data?.detail || e?.message || "漫画页面加载失败"; error.value = readerError.value; tab.value = "reader"; }
...
function handleReaderImageError(pageId: string) { readerImageErrors.value = { ...readerImageErrors.value, [pageId]: "页面内容加载失败，请重新打开章节或检查附件可用性。" }; }
