...
type AuthenticationView = {
  ...
  refreshToken: string;
  expiresAt: string;
  user?: { username?: string; displayName?: string; roleCodes?: string[] };
  permissions?: string[];
};
...
export const getLogin = async (data?: object): Promise<UserResult> => {
  const result = await http.request<AuthenticationView>("post", "/auth/login", {
    data
  });
...
export const registerUser = async (data: object): Promise<UserResult> => {
  const result = await http.request<AuthenticationView>(
    "post",
    "/auth/register",
    { data }
  );
...
export const refreshTokenApi = (data?: object) => {
  return http
    .request<AuthenticationView>("post", "/auth/refresh-token", { data })
    .then(toUserResult);
};
