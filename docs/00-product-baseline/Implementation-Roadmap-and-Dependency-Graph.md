...
优先完成 `登录授权 → 番剧导入 → Resource/Attachment → Search → 播放/进度 → Backup Restore Drill` 的纵向验收，再扩大专业 App 范围。ADR-008 的安全、事件、任务和测试门禁是该切片的前置条件。

首轮容量测试使用明确记录的 2 vCPU / 4 GiB 自托管基准环境（这是测试配置，不是已证明的最低生产要求），分别记录空闲/导入/播放/备份时的进程内存、数据库连接、任务并发、API p95/p99、首字节时间、Outbox 延迟和 RPO/RTO。记录数据集大小、Provider、并发数和结果；没有实际测量前不填写达标结论。默认 Worker 并发设为 1，可显式调高并重新测量。
