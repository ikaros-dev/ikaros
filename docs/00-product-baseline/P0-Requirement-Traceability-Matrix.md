...
| `FR-AUTH-01` Identity | Current User / JWT Validation / `identity.invalidate-user-tokens` | User + `security_version`；无 Login Session / Token Digest 登录态表 | `getCurrentUser`, `invalidateCurrentUserTokens` | `authentication.user.tokens-invalidated` + Authentication 事件 | `P0-ID-009~015` |
