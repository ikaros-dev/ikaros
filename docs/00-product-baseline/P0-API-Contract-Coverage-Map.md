...
`contracts/schema/p0-event-v1.schema.json` 冻结 P0 事件 Envelope、UUIDv7 Event ID、Schema Version、Producer Namespace，以及当前 **44 个 P0 v1 Event Type**。本次无状态 JWT 收敛删除 `authentication.session.revoked`、`authentication.user.sessions-revoked`，新增 `authentication.user.tokens-invalidated`；本次新增角色更新与删除事件。用户与 Token 失效事件由 Authentication 产生，角色与权限事件由 Authorization 产生。每种事件的 Payload 字段级约束仍由 `P0-Event-Payload-Schema-Registry.md` 管理，并应在 Phase 0 实现阶段扩展为机器可执行的兼容性检查（`P0-EVT-004/013/014`）。
