...
`POST /api/security/step-up` 发起 `LOGIN_STEP_UP` 时，服务端可以在同一账号最近一次 OTP 成功验证仍处于复用窗口内的情况下直接返回新的 `Verification Grant`，而不再次发送 OTP。复用窗口默认 4 小时，由应用配置决定。

命中复用窗口时，响应仍不得包含 OTP、OTP Digest 或认证邮箱；客户端使用响应中的 `verification_grant` 继续执行原操作。未命中窗口时，响应返回普通挑战摘要，客户端必须完成 OTP 验证后再使用返回的 Grant。

复用只适用于同一账号和 `LOGIN_STEP_UP` 用途，且新的 Grant 仍必须校验主体、`security_version`、`purpose`、`target_reference`（适用时）、SVL 与 `exp`。
