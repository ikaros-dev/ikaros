...
> 本文定义 Restore / Repair 的 Console 安全交互。页面归属遵循“资源 / 资源库”(`/resources/library/**`)、存储(`/storage/**`)和“资源 / 活动中心”(`/resources/activity/**`)，不建立独立 Restore Center。

## 1. 发起入口

Restore 可以从：

- `/resources/library/:resourceId` 的 Resource Availability；
- `/storage/archive` 的归档资源列表；
- `/storage/maintenance` 的高级诊断。

普通用户优先从业务 Resource 发起；技术维护人员才从 Blob / Replica 视角发起。

## 2. Restore Confirmation

确认必须展示：目标业务 Resource、当前状态、恢复来源层级、目标层级、预计数据量和异步语义。

如果已有可访问副本，应解释为什么仍需要恢复或禁止重复操作。

## 3. 状态

Resource / 资源项使用 Restoring 表达业务状态。执行细节进入 `/resources/activity/:activityId`。
...
Retry 必须遵守幂等语义，不能重复创建已完成副本或破坏已有可用资源。
...
- Restore 从 `/resources/library/:resourceId` 可理解地发起；
- 执行统一进入 `/resources/activity`，详情进入 `/resources/activity/:activityId`；
- 失败能回到 Resource / Storage 解释影响；
- Cancel / Retry 只在安全阶段出现；
- 不再使用旧 `/library`、`/activity` 作为 canonical Console 路由；
