...
public class LocalScanItem {
    ...
    private @Nullable MediaPhysicalType physicalType;
    /** 文件在本次扫描中的业务角色. */
    private @Nullable MediaRole role;
    /** 用于展示文件语言、用途和声道等信息的元数据. */
    @JsonProperty("display_metadata")
    private @Nullable Map<String, String> displayMetadata;
    /** 自动关联或待确认关联的候选主资源附件标识. */
    @JsonProperty("candidate_primary_attachment_id")
    private @Nullable UUID candidatePrimaryAttachmentId;
    /** 媒体容器中探测到的轨道列表. */
    private @Nullable List<MediaTrack> tracks;
    /** 当前文件媒体轨道探测失败的原因. */
    @JsonProperty("probe_failure_reason")
    private @Nullable String probeFailureReason;
}
