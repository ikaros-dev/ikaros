...
public class DriveController {
    ...
    @GetMapping("/nodes/{nodeId}") public Mono<DriveNodeView> node(@RequestHeader("X-Ikaros-Actor-Id") UUID actor,@PathVariable UUID nodeId){return service.node(actor,nodeId);}
    @GetMapping("/spaces/{spaceId}/children/page") public Mono<PageResponse<DriveNodeView>> childrenPage(@RequestHeader("X-Ikaros-Actor-Id") UUID actor,@PathVariable UUID spaceId,@RequestParam(required=false) UUID parentId,@RequestParam(defaultValue="0") int page,@RequestParam(defaultValue="20") int size){return service.childrenPage(actor,spaceId,parentId,page,size);}
    ...
    @PostMapping("/nodes/{nodeId}/revisions/{revisionNo}/actions/restore") public Mono<ResponseEntity<DriveNodeView>> restoreRevision(@RequestHeader("X-Ikaros-Actor-Id") UUID actor,@PathVariable UUID nodeId,@PathVariable long revisionNo,@RequestHeader(value="If-Match",required=false) String ifMatch){return service.restoreRevision(actor,nodeId,revisionNo,IfMatchVersion.parse(ifMatch)).map(view->withEtag(view));}
    ...
    @PostMapping("/bindings/{bindingId}/camera-backups/{cameraBackupId}/retry") public Mono<CameraBackupView> retryCameraBackup(@RequestHeader("X-Ikaros-Actor-Id") UUID actor,@PathVariable UUID bindingId,@PathVariable UUID cameraBackupId){return service.retryCameraBackup(actor,bindingId,cameraBackupId);}
    @PostMapping("/bindings/{bindingId}/camera-backups/scan") public Mono<CameraBackupScanView> scanCameraBackups(@RequestHeader("X-Ikaros-Actor-Id") UUID actor,@PathVariable UUID bindingId,@Valid @RequestBody CameraBackupScanRequest request){return service.scanCameraBackups(actor,bindingId,request);}
    @PutMapping("/bindings/{bindingId}/camera-backup-scope") public Mono<CameraBackupScopeView> cameraBackupScope(@RequestHeader("X-Ikaros-Actor-Id") UUID actor,@PathVariable UUID bindingId,@Valid @RequestBody CameraBackupScopeRequest request){return service.configureCameraBackupScope(actor,bindingId,request);}
